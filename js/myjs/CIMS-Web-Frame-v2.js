window.CIMS = (function(document, window) {
	...
	var CIMS_DEFEATED_FORMAT = /^DEFEATED\|+?[1-9][0-9]*$/;
	...
	function onDOMReady() { 
			 window.iframe = $$(options.containerid);
			 ...
			 if(options.version=="v3"){
				 var contentUrl = [options.host,'login2.html?view=frame&type=0&sign=', encodeURIComponent(cimsSig), '&parent=',
	                  encodeURIComponent(document.location.href), '&version=',options.version ,'&email=', options.email,'&token=',options.token,'&username=',options.username].join('');
	 
			 }
			 if(options.version=="v4"){
				 var contentUrl = [options.host,'login1.html?view=frame&type=0&sign=', encodeURIComponent(cimsSig), '&parent=',
	                  encodeURIComponent(document.location.href), '&version=',options.version ,'&email=', options.email,'&token=',options.token].join('');  
	 ...
	 function go(response) { 
	     ...
	     if(options.formid){
	         ...
	         //iframe.parentElement.insertBefore(form, iframe.nextSibling);//if ie8 error, please write off 
	     }
	 ...
	 function defeated(response){
	     //defeated!
	     if (typeof defeatedCallback === "function") {
	         defeatedCallback.call(null, null);
	     } else {
	         //alert(222);
	     }
	 }
	 ...
	 function onReceivedMessage(event) { 
			...
			if(isCIMSDefeatedMessage(event)){
				defeated(event.data);
				//offMessage(onReceivedMessage);
			}
	 }
	...
	function isCIMSDefeatedMessage(event) {
        return Boolean(
        		options.host.startWith(event.origin) &&
            typeof event.data === 'string' &&
            (
            		event.data.match(CIMS_DEFEATED_FORMAT)
            )
        );
    }
